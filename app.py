...
def render_zscore_range_controls(base_key):
    ...
    for i in range(int(num_z_ranges)):
        with st.expander(f"Z-score range {i + 1}", expanded=True):
            ...
            with st.expander("Range color", expanded=False):
                c4, c5, c6 = st.columns(3)
                with c4:
...
with st.sidebar:
    ...
    with st.expander("Event lines / shaded epochs", expanded=False):
        ...
        for i in range(int(num_events)):
            ...
            with st.expander(f"Event / epoch {i + 1}: apply to", expanded=False):
